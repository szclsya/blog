...
So, we still have to somehow downsample the audio file. Luckily, that can also be automated too. I just used ffmpeg to convert the the audio sample to 16bit, 48000Hz (which is probably the best quality the little recorder can accept). Since the script has become pretty long, I won't quote it here. You can find the exact script I use at [[https://github.com/szclsya/dotfiles/blob/master/bin/record_minidisc][here]].

* Done!
And that's it! Now, we can create an awesome MiniDisc mixtape with the correct track marker easily with a command. The only drawback for now is that the track name is still lacking, but it is not a huge deal for me.
