...
ZFS has a component called =ZED (ZFS event daemon)=, which, when configured correctly, will send emails to specified address when certain events (like scrubs, excess errors and more) happens. We will discuss the specific steps to setup ZED, as it requires setting up email forwarder on your system, and such is out-of-scope for this article. Check [[https://wiki.archlinux.org/title/ZFS#Monitoring_/_Mailing_on_Events][ZFS#Monitoring - ArchWiki]] for more.
