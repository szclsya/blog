...
for img in $(find ./img | grep "avif$"); do
    ...
    if [[ ! -e ${new_path/'.avif'/'.webp'} ]]; then
        magick "$img" -resize "$SIZE" -strip -quality 80 -define webp:lossless=false -define webp:method=6 "${new_path/'.avif'/'.webp'}"
    fi
    # Generate minified AVIF
    if [[ ! -e $new_path ]]; then
        magick "$img" -resize $SIZE -strip -quality 80 "$new_path"
    fi
done
...
for img in $(find ./img | grep "jpg$"); do
    ...
    if [[ ! -e ${new_path/'.jpg'/'.webp'} ]]; then
        magick "$img" -resize "$SIZE" -strip -quality 80 -define webp:lossless=false -define webp:method=6 "${new_path/'.jpg'/'.webp'}"
    fi
    # Generate AVIF
    if [[ ! -e ${new_path/'.jpg'/'.avif'} ]]; then
        magick "$img" -resize $SIZE -strip -quality 80 "${new_path/'.jpg'/'.avif'}"
    fi
done
...
for img in $(find ./img | grep "png$"); do
    ...
    # Copy original PNG
    if [[ ! -e $new_path ]]; then
        magick "$img" -resize "$SIZE" -strip "$new_path"
        # Use OptiPNG to minimize the result
        optipng "$new_path"
    fi
    # Generate lossless WebP
    if [[ ! -e ${new_path/'.png'/'.webp'} ]]; then
        magick "$img" -resize "$SIZE" -strip -define webp:lossless=true "${new_path/'.png'/'.webp'}"
    fi
    # Generate AVIF
    if [[ ! -e ${new_path/'.png'/'.avif'} ]]; then
        magick "$img" -resize "$SIZE" -strip "${new_path/'.png'/'.avif'}"
    fi
done
